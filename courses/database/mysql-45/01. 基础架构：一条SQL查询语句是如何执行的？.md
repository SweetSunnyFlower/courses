...
const props = defineProps({
  src: {
    type: String,
    default: 'mp3/01. 基础架构：一条SQL查询语句是如何执行的？.mp3'
  },
  title: {
    type: String,
...
const remain = computed(() => Math.max(0, duration.value - current.value))
...
<div :class="$style.player">
  ...
  <div :class="$style.controls">
    <button :class="$style.btn" @click="skip(-15)" title="后退 15 秒">«15</button>
    <button :class="$style.playBtn" @click="toggle">{{ playing ? '❚❚' : '▶' }}</button>
    <button :class="$style.btn" @click="skip(15)" title="前进 15 秒">15»</button>
  </div>

  <div :class="$style.progress" @click="seek">
    <div :class="$style.progressFill" :style="{ width: progress + '%' }"></div>
  </div>
...
.btn:hover { background: var(--vp-c-gray-soft, #f0f0f0); }
.playBtn {
...
.playBtn:hover { background: var(--vp-c-brand, #3451b2); color: #fff; }
.progress {
  height: 8px;
...
.rateBtn { padding: 3px 8px; font-size: 12px; }
.rateBtn.active {
  background: var(--vp-c-brand, #3451b2);
  color: #fff;
  border-color: var(--vp-c-brand, #3451b2);
