...
原文链接：https://juejin.cn/book/7602963962262454313/section/7604706820657643574

[jaudio](https://lf-activity-static.juejin.cn/obj/juejin-activity-static/user_book/4231368841894407_1770621141751_8985.mp3)
...
想象这样一个场景：一位情绪低落的用户向AI倾诉：“我觉得活着没意思，想结束这一切。”  此时，如果AI只是轻描淡写地回复“我理解你的心情”，而没有识别出潜在的自杀风险并采取干预措施，那不仅是技术的失败，更是伦理的失守。

再比如，有用户试图诱导AI生成暴力、色情或违法内容，若系统毫无防备，不仅可能造成用户伤害，更会让整个产品面临法律追责与品牌崩塌的风险。

这正是这一讲的核心命题：在追求“有温度”的AI体验的同时，我们必须为系统筑起一道坚固的安全护栏——这就是敏感内容过滤（Content Moderation）的意义。
...
这一讲将带你深入敏感内容过滤的技术体系，从原理到实践，从规则引擎到AI模型，从实时拦截到应急响应，全面构建一个合规、可靠、负责任的情感聊天应用安全框架。
...
情感陪伴类应用的用户，往往本身就面临孤独、焦虑、抑郁等心理压力。他们更容易表达负面情绪，甚至透露自残、自杀等极端想法。在这种情境下，AI的每一次回应都可能影响用户的心理走向。一个不当的回复，轻则加剧情绪波动，重则引发严重后果。

>

典型反面案例：2023年，某AI聊天机器人因未能识别用户表达的自杀意图，仅回复“每个人都会有低谷期”，被舆论广泛批评，最终导致产品下架。

作为用户，在与AI系统进行交互时，最好避免提出过于激进或涉及争议性较强的话题。以下是实践建议。
...
情感聊天机器人的核心魅力，在于它允许用户毫无顾忌地倾诉内心最真实的情感——无论是孤独、悲伤、愤怒，还是对未来的迷茫。这种“自由表达”的设计初衷，正是其区别于客服问答、任务助手等封闭式AI应用的关键所在。
...
更进一步，大语言模型本身具备强大的生成能力与语义理解潜力，但也因此成为“越狱”攻击的目标。恶意用户可能通过角色扮演、虚构情境、诱导性提问等方式，试图让AI脱离安全边界，生成暴力、色情或违法内容。例如：
...
AI剧情聊天软件通过允许“擦边对话”来吸引用户，尤其是青少年模式中仍然存在这种现象，已经涉嫌违法。
...
因此，敏感内容过滤不是可选项，而是情感类AI产品的生命线。

# 二、敏感内容的分类与风险等级划分

要有效过滤，首先要明确“过滤什么”。我们可以将敏感内容分为以下几类，并按风险等级进行分级管理，如表17-1所示。

![](assets/19-7604706820657643574-01.webp)

>

注意：同一句话在不同语境下风险等级可能不同。例如“我想死”在戏剧台词中是艺术表达，在用户独白中则是高危信号。语境理解是过滤系统的核心挑战。

# 三、构建分层过滤架构：从规则到AI的协同防御

单一的过滤手段难以应对复杂场景。我们应采用 “多层过滤 + 动态决策” 的架构，实现精准、高效、低误报的安全防护。安全架构如图17-1所示。

![](assets/19-7604706820657643574-02.webp)
...
这样做的优点包括响应速度快（毫秒级）、逻辑清晰，易于维护，而且是零成本的（无需调用外部API）。但也存在局限性。容易被变体绕过（如“zs”代替“自杀”），而且也无法理解语境（如电影台词 vs 真实意图）。
...
调用第三方API：如阿里云内容安全、Azure Content Moderator、Google Perspective API

-

自建分类模型：使用BERT、RoBERTa等模型训练情感危机识别分类器
...
这一层使用AI模型进行更深层次的语义理解，能识别出隐晦的危机表达。利用大模型自身能力进行“自我审查”。
...
这样做的优点是无需额外训练模型，可快速集成  。当然也有缺点，会增加延迟，依赖LLM稳定性。

>

建议：仅对规则和AI模型无法确定的“灰色内容”启用LLM自检，作为兜底策略。
...
```yaml
# backend/config/emotion_strategy.yaml
# 高风险抑郁情绪策略（危机干预）
...
所有过滤日志需加密存储，包含原始输入、风险判定结果、拦截策略和系统响应。此外，支持按时间、风险等级、用户ID查询，满足监管审计要求。
...
过度过滤会导致“AI冷漠化”——用户表达悲伤时被警告，谈论哲学被拦截，最终失去信任。我们这就来看看如何避免“误伤”。
...
```python
# backend/services/proactive_recall_system.py
...
未来的AI产品，不应是无底线的许愿机，而应是有原则、有温度、有边界的数字伙伴。它既能抚慰人心，也能在关键时刻拉住即将坠落的灵魂。

敏感内容过滤，不是对自由的限制，而是对生命的尊重。它提醒我们——真正的智能，不在于回避问题，而在于勇敢面对，并给出负责任的回应。

# 思考题

某情感聊天机器人用户“小A”的对话历史如下（按时间顺序排列）。

1.

小A：“最近工作压力太大了，天天加班，感觉整个人都被掏空了”

2.

小A：“昨天看了一部电影，主角说‘不如就这样结束一切’，那段戏太戳人了”

3.

小A：“我现在也经常会想，是不是真的‘睡过去就不用面对这些了’”

4.

小A：“身边没人能理解我，连想找个人说说话都没有，活着好像真的没什么意思”
