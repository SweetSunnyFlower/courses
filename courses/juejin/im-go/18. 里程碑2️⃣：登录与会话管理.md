...
原文链接：https://juejin.cn/book/6963277002044342311/section/6973289197750730792

![chapter18.png](assets/18-6973289197750730792-01.webp)

## 里程碑

本章完成之后就到达第二个`里程碑`。它的意义在于：

- 从SDK与网关建立连接到`握手登陆`，再到逻辑服务，整个消息的处理链路已经全部打通，之后《控制层》其它逻辑就基本与底层无关了。

[登录登出模块 v1.2](https://github.com/klintcheng/kim/tree/v1.2)

![image.png](assets/18-6973289197750730792-02.webp)
...
![login时序.png](assets/18-6973289197750730792-03.webp)
...
![image.png](assets/18-6973289197750730792-04.webp)

图中的`VERIFY SIGNATURE`有一个信息要注意下：`your-256-bit-secret`，也就是密钥，这个在网关中验证Token时，必须使用一个与创建token时相同的密钥，使用HS256算法对Hander和payload做相同的签名计算，得到的值与JWT中的签名做对比。

在传统的token方案中，token表示的`声明`是存储在缓存中（如redis），因此校验时必须根据token从缓存中取出声明信息。那么JWT与传统的token方案相比有什么优势和缺点呢？
...
![login_flow.png](assets/18-6973289197750730792-05.webp)
...
在这个登陆的处理器中，使用到了`SessionStorage`会话管理接口中的两个方法：会话查询与添加。这里需要需要注意的地方就是：
...
原因就是发送消息时`寻址的索引`使用的是用户标识Account，如下图：

![会话分层.png](assets/18-6973289197750730792-06.webp)

而Session存储时的索引是channelId，在收到LogicPkt读取发送方会话时就是使用Handler中的channelId属性从会话管理中查询，如果查不到就说明会话丢失，需要重新登录。

而且发送消息时`寻址`只需要ChannelId与GateId两个信息即可定位到网关与网关中的channel，在高频的消息发送时，特别是存在群的寻址扩散情况下，就要尽量提高寻址的速度，在不优化外部逻辑的情况下，我们可以从两个方面做优化：
...
![location_store.png](assets/18-6973289197750730792-07.webp)

location采用的私有的序列化方法，我们在[网络传输详解一章](https://juejin.cn/book/6963277002044342311/section/6964230973521133571)做过基准测试的对比，它效率最高，零分配。

![session_store.png](assets/18-6973289197750730792-08.webp)
...
![image.png](assets/18-6973289197750730792-09.webp)
