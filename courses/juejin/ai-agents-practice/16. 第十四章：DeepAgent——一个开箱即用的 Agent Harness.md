# 第十四章：DeepAgent——一个开箱即用的 Agent Harness

原文链接：https://juejin.cn/book/7626976407423303731/section/7627371176324038665

本章demo地址：[feat/deepagents](https://github.com/Cookieboty/autix-demo/tree/feat/deepagents)

第八章到第十三章依次解决了 Agent 工程中的几个基础问题：用 LangGraph 描述运行流程，组织多 Agent 协作，控制上下文与 Token 成本，通过 RAG 接入业务知识，通过 MCP 标准化工具调用，以及将可复用方法沉淀为 Skill 资产。

在这些实践之后，Agent 工程中的另一类成本逐渐显现：许多代码并不直接服务于业务逻辑，而是在重复装配运行时基础设施：
...
这些内容共同构成 Agent 的运行环境。如果每创建一个新 Agent 都从零装配，工程成本、测试成本和维护成本都会被放大。

`deepagents` SDK 面向的就是这一层问题。它不是新的大模型，也不是新的图原语，而是一套 Agent Harness（运行时框架）：将任务规划、虚拟文件系统、子 Agent 调度、上下文压缩、Skills 接入等通用能力，预装为一组默认中间件。开发者仍然负责定义业务工具和 system prompt，但不必为每个 Agent 重写同一套运行时脚手架。

> RAG 负责引入知识，MCP 负责标准化工具调用，Agent 负责自主执行任务，Skills 负责沉淀方法资产，DeepAgent 则将这些能力组织成一套可复用的 Agent Harness。

本章首先定位 DeepAgent 在 Agent Harness 谱系中的位置，并回答两个工程取舍问题：已有 Claude Agent SDK / Pi 时，为什么仍然需要 LangChain；已有 LangChain 时，为什么仍然需要 DeepAgent。随后依次展开最小可运行示例、需求分析 Agent、内置规划工具、虚拟文件系统、子 Agent 和 Skills 接入。
...
- 用“需求分析 Agent”场景观察 DeepAgent 相对 `createReactAgent` 的行为差异
...
为保持与前几章的连续性，本章继续使用需求分析系统作为主干场景。示例中不修改第八、九章已有的需求分析代码，而是用 DeepAgent 重新包装同一组业务工具。DeepAgent 接入已有 LangGraph 图的方式，将在第十五章展开。
...
## 14.1 Agent Harness 谱系中的 DeepAgent

![](assets/16-7627371176324038665-01.webp)

Agent Harness 的四类运行时要素：Guides、Sensors、Loop Control 与 Hardening

进入 DeepAgent 的代码示例之前，需要先将它放回 Agent 工具谱系中理解。到 2026 年，Agent 产品和框架已经形成较清晰的分层：面向终端交互的 CLI、面向编码任务的 SDK，以及面向通用长任务的运行时框架。DeepAgent 的定位，应在这一谱系中判断。

### 14.1.1 Harness Engineering：Agent = Model + Harness

近几年，“如何让大模型稳定完成任务”的工程重心经历了三次迁移：
...
进入 Harness Engineering 阶段后，一个核心判断可以概括为：
...
DeepAgent 的定位是：将长任务中最常见的运行时要素预装为默认中间件，使开发者将主要精力放在业务能力定义上，而不是重复搭建基础设施。

### 14.1.2 三个 Harness 横向对比

将 DeepAgent 与 Claude Agent SDK、Pi 横向对比，可以看到三者在定位、内置能力和绑定程度上的差异：
...
除了“内置能力多少”之外，还要判断它是否适合作为库嵌入业务系统：
...
这条演进线的目标，是将重复脚手架从业务代码中抽离出来：先提供图运行时，再提供中间件机制，最后提供一套默认装配好的运行时。

### 14.2.2 为什么有了 Claude Agent SDK / Pi，还需要 LangChain

Claude Agent SDK 和 Pi 都能用于构建 Agent，但默认场景与约束不同。
...
LangChain 的价值在于提供 provider 无关、不预设业务场景的可编程地基。同一套业务逻辑，可以根据复杂度选择不同粒度：
...
第八章到第十三章已经用 LangChain / LangGraph 手写过多类运行时能力：
...
DeepAgent 的作用，是将这组长任务高频能力预装为默认中间件：
...
`createDeepAgent` 返回的仍然是一个编译后的 LangGraph 图，因此可以继续使用 `invoke()`、`stream()`、`streamEvents()`、Checkpointer 和 LangSmith trace。DeepAgent 并不替代 LangGraph，而是在 LangGraph 之上提供一套默认装配。

> `createAgent` 提供 middleware 机制，DeepAgent 提供一组已经装配好的 middleware 默认值。前者是框架能力，后者是长任务运行时模板。

## 14.3 DeepAgent 的主要功能

![](assets/16-7627371176324038665-02.webp)
...
这些能力都通过 Middleware 实现。每个 middleware 可以承担三类职责：
...
需要注意的是，DeepAgent 没有引入新的 LangGraph 原语。`write_todos` 是一个普通 Tool，虚拟文件系统是一组文件工具，`task` 是在主 Agent 内部启动子 Agent。第八、九章讲过的 State、Node、Edge、条件边和子图，仍然是底层实现。
...
> 关于 `systemPrompt`：你传入的业务 prompt 不会覆盖内置 prompt，而是与之组合——DeepAgent 先提供引导模型使用规划、文件系统和子 Agent 的内置基础 prompt，再拼接你的业务 prompt。因此当业务 prompt 出现“直接回答、不要调用工具”这类指令时，可能与内置规划引导冲突，导致 `write_todos` 不被触发（参见 14.6.2）。
...
先通过一个最小示例验证 `createDeepAgent` 的基本运行方式。
...
const agent = createDeepAgent({
  ...
  tools: [getWeather],
  systemPrompt:
    "你是一个天气助手。用户问天气时，调用 get_weather 工具获取数据。",
});
...
完整脚本见 `scripts/run-deepagent-hello.ts`，运行方式：
...
![](assets/16-7627371176324038665-03.webp)
...
但在天气查询这类简单任务中，模型通常会直接调用业务工具并返回结果，不会生成 todos，也不会写文件。这属于正常行为：DeepAgent 提供能力，但不强制每个任务都使用全部能力。

### 14.4.4 `invoke()` 的返回值结构

`agent.invoke()` 返回的对象包含以下关键字段，后续章节会逐一用到：

```tsx
const result = await agent.invoke({ messages: [...] });

result.messages   // BaseMessage[]  — 完整的消息历史（含工具调用和返回）
result.todos      // Todo[]         — 当前任务计划，每项含 id / content / status
result.files      // Record<string, string> — 虚拟文件系统中的文件（路径 → 内容）

```

| 字段 | 类型 | 来源中间件 | 说明 |
| --- | --- | --- | --- |
| `messages` | `BaseMessage[]` | LangGraph 核心 | 完整消息链，最后一条通常是 Agent 的最终回复 |
| `todos` | `{ id, content, status }[]` | `TodoListMiddleware` | 模型通过 `write_todos` 维护的任务计划；短任务通常为空数组 |
| `files` | `Record<string, string>` | `FilesystemMiddleware` | 虚拟文件系统中写入的文件；使用 `StateBackend` 时执行结束即消失 |

这三个字段分别对应 14.6（`todos`）、14.7（`files`）和常规 LangGraph 输出（`messages`）。理解它们的来源，有助于判断“为什么某个字段为空”：通常是因为模型没有调用对应的中间件工具。

## 14.5 需求分析 Agent：面向业务场景的 DeepAgent 示例

天气查询类任务流程较短，难以体现 DeepAgent 的运行时价值。更适合作为对照的场景是“需求分析”：

> 对一个产品需求进行专业分析：完整性检查、复杂度估算、模块拆解、风险识别，输出一份结构化分析报告。

这个任务会同时暴露三类需求：

1. 需要规划：先检查完整性，再估算复杂度，然后拆解模块和识别风险，最后形成报告。

2. 中间产物多：每个分析维度的结果需要暂存，不能全部堆在 messages 中。

3. 可以拆子任务：不同维度的分析可以交给子 Agent 独立完成。

这三类需求分别对应 DeepAgent 的 `write_todos`、虚拟文件系统和 `task` 子 Agent。
...
生成代码 Prompt：需求分析 Demo 脚本

```
在 services/chat/scripts/ 下创建 run-deepagent-analysis-demo.ts：
...
3. 注册工具：
- analyze_completeness，对应 requirement-analysis/scripts/analyze_completeness.py
- estimate_complexity，对应 requirement-analysis/scripts/estimate_complexity.py
...
- Python 工具脚本路径指向 src/skills/definitions/requirement-analysis/scripts/
...
const agent = createDeepAgent({
  ...
  systemPrompt:
    "你是一位资深需求分析专家。对用户提交的需求进行完整性分析和复杂度评估，输出结构化分析报告。",
});
...
完整脚本见 `scripts/run-deepagent-analysis-demo.ts`，运行方式：
...
这一版只注册业务工具，暂不接入 Skills，对应配套测试中的 Layer 2 端到端用例。
...
```
analyze_completeness → estimate_complexity

```

对于两步即可完成的分析任务，模型可能跳过显式规划，直接调用业务工具。`write_todos` 更常出现在多步骤、需要拆解的复杂任务中，例如第十五章的跨需求联合分析。工具调用链会随模型、提示词和任务复杂度变化，不宜作为固定断言。

![](assets/16-7627371176324038665-04.webp)
...
边界：DeepAgent 不适合替代所有 Agent。天气查询、简单分类、单工具调用，使用 `createReactAgent` 已经足够。只有当任务需要规划、上下文管理或子任务隔离时，DeepAgent 的优势才会体现出来。
...
{
  todos: [
    { id: "1", content: "分析需求完整性", status: "in_progress" },
    { id: "2", content: "估算技术复杂度", status: "pending" },
    { id: "3", content: "模块拆解与风险识别", status: "pending" },
    { id: "4", content: "输出分析报告", status: "pending" },
  ];
}
```
...
边界：`write_todos` 是引导式规划，不是强制流程。DeepAgent 的内置 prompt 会鼓励模型在复杂任务中先规划；但如果业务 prompt 明确要求“直接回答，不调用工具”，模型可能跳过规划。任务过于简单时跳过规划也属于正常行为。

### 14.6.3 验收方式

独立 demo 脚本 `scripts/run-deepagent-todos-demo.ts`，专门验证 `write_todos` 规划行为：
...
1. 用一个分三阶段的需求分析任务（完整性检查 → 复杂度估算 → 综合报告）触发 Agent

2. 打印调用链，观察 `write_todos` 是否出现

3. 打印 `result.todos` 中每项的 `status` 和 `content`

典型输出：

```
🔧 调用链: write_todos → analyze_completeness → estimate_complexity → write_todos
📋 todos: 3 项
[completed] 调用完整性检查工具分析需求缺失信息
[completed] 调用复杂度估算工具评估技术复杂度和工期
[in_progress] 整合结果并输出结构化分析报告
...
```bash
bun test test/chapter14-deepagent.spec.ts --test-name-pattern "14.6"

```

![](assets/16-7627371176324038665-05.webp)

若 `write_todos` 未出现，按以下顺序排查：

1. 任务是否过于简单，模型判断不需要规划（天气查询不会触发，多步骤分析更容易触发）

2. 业务 system prompt 是否与内置规划引导冲突（如包含”直接回答”类指令）
...
![](assets/16-7627371176324038665-06.webp)

虚拟文件系统把长任务中的中间产物从对话上下文迁移到可读写文件中

### 14.7.1 为什么需要文件系统

第十章讨论 Token 经济学时已经说明：上下文窗口不是无限资源。一个需求分析任务如果评估 5 个需求，每个需求的分析产生 2000 字信息，全部放入 messages 会带来三个问题：
...
DeepAgent 的虚拟文件系统提供了一个中间层：将阶段性资料写入文件，并在需要时读取。这样 messages 保留执行轨迹，文件系统保存中间产物。

> 如何量化收益：本章的对比以定性描述为主。若要获得实际数字，可在同一分析任务上分别使用 `createReactAgent` 和 `createDeepAgent` 执行一次，从 LangSmith trace 或回调中读取每轮 prompt tokens，对比“总 token 数 / 轮次 / 成本”。通常需求越多、中间产物越大，文件系统对上下文的节省越明显。这与第十章的成本拆账方法一致。
...
1. 分析完整性 → 写入 `/analysis/completeness.md`

2. 估算复杂度 → 写入 `/analysis/complexity.md`

3. 识别风险 → 写入 `/analysis/risk-assessment.md`
...
独立 demo 脚本 `scripts/run-deepagent-vfs-demo.ts`，专门验证虚拟文件系统的写入和读取：
...
该脚本会明确要求 Agent 将分析结果写入 `/analysis/completeness.md` 和 `/analysis/complexity.md`，再读回汇总。典型输出：

```
🔧 调用链: analyze_completeness → estimate_complexity → write_file → write_file → read_file → read_file
📁 files: 2 个
/analysis/completeness.md (856 字符): 完整性评分：67/100 ...
/analysis/complexity.md (423 字符): 复杂度：L ...
...
2. `📁 files` 包含两个目标路径及其内容摘要

3. 最终报告使用了文件中的内容，而不是只复述工具结果

配套测试：

```bash
bun test test/chapter14-deepagent.spec.ts --test-name-pattern "14.7"

```

![](assets/16-7627371176324038665-07.webp)

### 14.7.5 自动 Summarization：与文件系统互补的上下文管理

虚拟文件系统属于“主动”的上下文管理：由模型决定把什么写入文件、什么时候读取。`SummarizationMiddleware` 则是“被动”的兜底：当消息历史增长到阈值时，自动将较早的消息压缩成摘要，只保留摘要和最近若干条原始消息。两者互补——文件系统决定哪些中间产物不进入上下文，summarization 负责在历史仍然变长时防止超出窗口。
...
验收方式：summarization 在短任务中不会触发，难以稳定复现。要观察这一机制，可以构造一个工具返回内容较长、且需要多轮调用的任务，运行后对比压缩前后 messages 的长度与条数；也可以在中间件配置中调低触发阈值，再检查历史是否被替换为摘要。

## 14.8 Subagent：`task` 工具

![](assets/16-7627371176324038665-08.webp)

主 Agent 将子任务委托给多个独立子 Agent，并汇总返回结果

### 14.8.1 为什么需要子 Agent

在需求分析任务中，每个需求都可能需要多次工具调用、资料整理和判断。如果全部在主 Agent 的 messages 中完成，会出现三个问题：

1. 上下文膨胀：多个需求的分析过程都进入主上下文。

2. 信息干扰：后续需求分析会被前面需求的细节污染。

3. 失败传播：单个子任务错误可能影响主任务继续推进。

DeepAgent 的 `task` 工具用于解决这个问题：主 Agent 将子任务委托给子 Agent，子 Agent 在独立上下文中执行，并将结果摘要返回主 Agent。
...
const requirementAnalyst: SubAgent = {
  ...
  systemPrompt:
    "你是需求分析专家。请对指定需求进行完整性检查、复杂度估算和风险评估。",
  tools: [analyzeCompleteness, estimateComplexity],
};
...
const agent = createDeepAgent({
  ...
  systemPrompt:
    "你是需求分析专家。需要深入分析单个需求时，委托给 requirement-analyst。",
});
```

关于子 Agent，还需要说明三点：

- 工具继承：声明专用 `SubAgent` 时，它使用自己 `tools` 字段里列出的工具；如示例中 `requirement-analyst` 显式获得了两个分析工具。若不显式声明，子 Agent 能用哪些工具取决于 SDK 默认策略，建议总是为子 Agent 显式声明所需工具，避免依赖隐式继承。

- 执行方式：`task` 默认是串行委托——主 Agent 发起一次子任务、拿到摘要后再发起下一次。若需要并行委托，应以当前 `deepagents` 版本的能力为准，不要假设默认并行。

- `SubAgent` 与 `CompiledSubAgent` 的区别：本节使用的 `SubAgent` 是声明式接口（提供 `name` / `description` / `systemPrompt` / `tools`，由 DeepAgent 内部组装成可运行的 Agent）。第十五章会用到 `CompiledSubAgent`，它接受一个已编译的 `runnable`（任意 `Runnable`），适合把现有 LangGraph 图直接接入——代价是需要自己写适配器处理输入/输出契约。

### 14.8.3 `task` 的验收方式

与虚拟文件系统类似，`task` 不一定在普通任务中自动触发。要稳定验证，应将任务设计成天然可拆分的形式：
...
2. 如果声明了 `requirement-analyst`，检查 `task` 入参是否选择该子 Agent
...
两者不是替代关系。固定多专家并行适合 LangGraph `StateGraph`；动态委托适合 DeepAgent `task`。第十五章会展示如何将已有 LangGraph 图作为 DeepAgent 的子 Agent 接入。

### 14.8.3 `task` 的验收方式

独立 demo 脚本 `scripts/run-deepagent-task-demo.ts`，专门验证子 Agent 委派：
...
该脚本声明一个 `requirement-analyst` 子 Agent，给两个需求并要求分别委派。典型输出：
...
2. `🔀 task 委派次数` 大于 0，表明子 Agent 被调用

3. 主 Agent 的最终输出是一份汇总对比报告，不包含子 Agent 的完整执行过程

配套测试：

```bash
bun test test/chapter14-deepagent.spec.ts --test-name-pattern "14.8"

```

![](assets/16-7627371176324038665-09.webp)

## 14.9 把第十三章的 Skills 接入 DeepAgent

第十三章使用 `createReactAgent` + 手写 `load_skill` 工具演示了 Skills 按需加载。DeepAgent 内置 `SkillsMiddleware`，可以直接扫描 `SKILL.md` 目录，不再需要手写 `load_skill` 工具。
...
const agent = createDeepAgent({
  ...
  skills: [SKILLS_DIR],
  systemPrompt:
    "你是一位资深需求分析专家。需要专业能力时，可以加载对应 skill，并输出结构化分析报告。",
});
```
...
function callPythonTool(
  ...
  return execSync(
    `python3 "${join(SKILLS_DIR, skillName, "scripts", scriptName)}"`,
    {
      input: JSON.stringify(input),
      encoding: "utf-8",
    },
...
const agent = createDeepAgent({
  ...
  skills: [SKILLS_DIR],
  systemPrompt:
    "你是一位资深需求分析专家。需要专业能力时，可以加载对应 skill；输出结构化的需求分析报告。",
});
```
...
```
read_file → analyze_completeness → estimate_complexity
...
| 分层 | 验证点 | 预期 |
| --- | --- | --- |
| Layer 1 | Python 工具 JSON in/out | `analyze_completeness` 返回完整性评分，`estimate_complexity` 返回复杂度估算 |
...
| Layer 2 | 需求分析 Agent 端到端 | Agent 调用 `analyze_completeness`，输出包含”需求/分析/完整性”，长度大于 300 |
...
![](assets/16-7627371176324038665-10.webp)
...
> 简单任务适合 `createReactAgent`，强控制流程适合 LangGraph `StateGraph`，需要规划、上下文管理和子任务隔离的长任务适合 `createDeepAgent`。三者不是替代关系，而是面向不同复杂度的工程选择。

### 后续章节预告

第十五章进入 DeepAgent 长链任务与自主规划。本章使用需求分析系统的主干场景演示了 DeepAgent 的核心能力。下一章重点处理两个更接近生产环境的问题：

1. 如何把 DeepAgent 接入已有业务系统：将第八、九章的 `createAnalysisGraph()` 作为子 Agent，让 DeepAgent 编排多需求联合分析。
