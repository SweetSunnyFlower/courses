...
systemPrompt: '你是一位资深需求分析专家。对用户提交的需求进行完整性分析和复杂度评估，输出结构化分析报告。',
});
...
skills: [SKILLS_DIR],
systemPrompt: '你是一位资深需求分析专家。需要专业能力时，可以加载对应 skill，并输出结构化分析报告。',
});
...
backend: new FilesystemBackend({ rootDir: '/' }),
skills: [SKILLS_DIR],
systemPrompt: '你是一位资深需求分析专家。需要专业能力时，可以加载对应 skill；输出结构化的需求分析报告。',
});
