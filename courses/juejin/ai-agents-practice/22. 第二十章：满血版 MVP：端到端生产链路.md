# 第二十章：满血版 MVP：端到端生产链路

原文链接：https://juejin.cn/book/7626976407423303731/section/7627371359232131122

![generated-image-1782745060369.png](assets/22-7627371359232131122-01.webp)
...
![image.png](assets/22-7627371359232131122-02.webp)

本章的六个接线点依次是：hybrid RAG 检索升级、检索内容注入报告生成、MCP 工具接入、Skills 方法论注入、DeepAgent 长链路由、对话历史注入。后面的测试分层会围绕这六点逐一验证。

![generated-image-1782745059011.png](assets/22-7627371359232131122-03.webp)
...
`try/catch` 对这种「不抛错的挂起」无能为力。修复方式是在检索外层增加硬超时：一旦超过阈值，就降级为空上下文，主链路继续执行：
...
Controller 在持久化当前消息之前取最近 N 轮，拼成历史块前置到 `input`：
...
前端不需要区分主图和 DeepAgent。两条分支经过 orchestrator 转换后，都会输出同一套 SSE 协议。前端只需要稳定处理 `markdown/ui/progress/meta/artifact_created/done/error` 七种事件类型。
...
![generated-image-1782745065536.png](assets/22-7627371359232131122-04.webp)
...
describe("20.6 长链路由 detectLongChain", () => {
  it("多工单输入（≥2 个不同 REQ）路由到 DeepAgent", () => {
    expect(detectLongChain("评估 REQ-001/REQ-002/REQ-003 的总体影响")).toBe(
      true,
    );
...
### Layer 2：真实 LLM 后端链路（需 OPENAI_API_KEY，无 key 自动跳过）
...
describe("20.6 长链输入端到端路由到 DeepAgent", () => {
  it("多工单输入触发 DeepAgent 分支并返回非空报告", async () => {
    ...
    let report = "";
    for await (const ev of orch.streamOrchestrate(
      input,
      "无相关参考文档",
      undefined,
    )) {
      if (ev.type === "log" && ev.message.includes("DeepAgent"))
        routedToDeep = true;
...
Layer 2 验证的是后端行为——注入特征事实后报告正文出现对应术语（证明检索→报告数据流真正闭合），多工单输入走 DeepAgent 分支并产出非空报告。
...
它不覆盖的两条路径要说清楚，避免误以为「一条 E2E 测全了」：
...
| 步骤 | 截图 |
| --- | --- |
| ① 登录页：填入账号密码，准备走 user-system 鉴权 | ![01-login.png](assets/22-7627371359232131122-05.webp) |
| ② 登录后进入工作区（左对话区 + 右 Artifact 区） | ![02-workspace.png](assets/22-7627371359232131122-06.webp) |
| ③ 发送需求后：用户气泡 + `ThinkingIndicator` 流式分析中 | ![03-streaming.png](assets/22-7627371359232131122-07.webp) |
| ④ 终态：右侧 `ArtifactPanel` 渲染出需求分析报告（含 OAuth2 / corpId 校验 / 安全风险） | ![04-artifact.png](assets/22-7627371359232131122-08.webp) |
...
三个 watch/dev 编译器同时常驻，本身就会占用大量内存；再叠加一条完整 E2E，在一台同时运行浏览器、IM、笔记应用的 16G 机器上，很容易出现内存被压满 → 频繁触发 swap（内存换页）→ 系统假死或重启。这里的根因是资源耗尽，不是测试逻辑或被测代码错误。
...
1. dev 模式适合改代码，不适合跑 E2E。 watch / HMR（Hot Module Replacement，热模块替换）为了热更新会常驻编译器并占用较多内存，而 E2E 不需要这部分开销。E2E 应当对着构建产物跑，既更省资源，也更接近线上真实形态。
...
console.log(
  `20.3 校验：报告是否消费了检索内容 → ${ragHit ? "✅ 是" : "❌ 否"}`,
);
```
...
![image.png](assets/22-7627371359232131122-09.webp)
...
![image.png](assets/22-7627371359232131122-10.webp)
...
![generated-image-1782745062050.png](assets/22-7627371359232131122-11.webp)
...
![image.png](assets/22-7627371359232131122-12.webp)
...
![generated-image-1782745061000.png](assets/22-7627371359232131122-13.webp)
...
![image.png](assets/22-7627371359232131122-14.webp)
