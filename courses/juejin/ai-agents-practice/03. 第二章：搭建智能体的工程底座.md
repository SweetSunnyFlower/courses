...
原文链接：https://juejin.cn/book/7626976407423303731/section/7627370512372465715

![image.png](assets/03-7627370512372465715-01.webp)

本章分支[feat/foundation](https://github.com/Cookieboty/autix-demo/tree/feat/foundation)
...
![Gemini_Generated_Image_8pptyf8pptyf8ppt.png](assets/03-7627370512372465715-02.webp)
...
![Gemini_Generated_Image_qrup6xqrup6xqrup.png](assets/03-7627370512372465715-03.webp)
...
- `services/chat`：Nest Chat 服务（提供一个 `/health` 与一个 `/hello`）

- `clients/chat-web`：Next Web（页面渲染 + 调用 Chat 服务）

- `infra/compose`：Docker Compose（一次命令启动 chat-web + chat）

这条链路的设计意图是：先把工程骨架和联通方式跑通。后续无论接不接模型、接什么工具、引入什么运行时，都可以在这个骨架上逐步加能力，而不是一开始就被“选型细节”和“项目模板”困住。

✅ 最终验收标准

- 浏览器打开 `http://localhost:3002`，能看到页面展示的 `APP_NAME`

- 点击按钮后，页面能调用 Chat 服务并显示返回的字符串

- 用 Compose 一条命令能同时启动 chat-web 与 chat
...
![image.png](assets/03-7627370512372465715-04.webp)
...
- `packages/contracts` 能被 chat-web 与 chat 同时引用，确保“同一份类型/规则/常量”不被重复定义

换句话说：这一节先把“仓库能稳定运转”这件事搞定，后面 2.5/2.6 才能专注在 Web 和 Chat 服务本身。
...
![image 1.png](assets/03-7627370512372465715-05.webp)
...
- `workspaces` 把本地包依赖关系收敛到一处，减少路径漂移
...
"scripts": {
"clean:ports": "for port in 3002 4001; do lsof -ti :$port | xargs kill -9 2>/dev/null; done; echo 'Ports cleaned'",
...
Turbo 需要用 `tasks/dependsOn/outputs` 把 build、dev、typecheck 等任务组织成“任务图”，并且划定缓存边界。这样后续工作区变多时，才能增量执行、复用缓存，而不是每次全量重跑。
...
把 TypeScript 的严格模式、模块解析策略、以及共享包的路径别名统一到根目录，避免 web/chat/共享包各自为政。后续加新包时，只需要 extends 这份基线即可。
...
"lib": ["ES2022"],
"paths": {
"@autix/contracts": ["./packages/contracts/src/index.ts"],
"@autix/contracts/*": ["./packages/contracts/src/*"]
}
}
}
...
这是为了让 Bun workspaces 把这个目录识别为一个可依赖的包，并通过 `exports` / `types` 暴露共享入口。
...
为什么要单独一份：共享包需要有自己的编译边界，让 web/chat 在编译期拿到类型信息。把配置放在包内，也方便后续复用同样的模板新建其他共享包。
...
为什么先放一个常量：它最容易验证“共享包是否被正确引用”。只要 web 和 chat 都能读到同一个 `APP_NAME`，就说明 workspace 依赖、构建链路和路径解析是通的。
...
![image 2.png](assets/03-7627370512372465715-06.webp)

✅ 验收标准

- 根目录执行 `bun install` 不报错

- 根目录执行 `bun run build` 时，后续的 `@autix/chat-web` / `@autix/chat` 能解析到本地 workspace 包 `@autix/contracts`

- 参考资料

- Bun workspaces: [https://bun.com/docs/pm/workspaces](https://bun.com/docs/pm/workspaces)

- Bun catalogs: [https://bun.com/docs/pm/catalogs](https://bun.com/docs/pm/catalogs)

- Bun isolated installs: [https://bun.com/docs/pm/isolated-installs](https://bun.com/docs/pm/isolated-installs)

- Turbo tasks 配置: [https://turborepo.com/docs/crafting-your-repository/configuring-tasks](https://turborepo.com/docs/crafting-your-repository/configuring-tasks)

## 2.5 客户端：初始化 Next 并使用共享包

这一节的目标是：

- 初始化 `clients/chat-web`，让 Next 项目在 monorepo 里以 workspace 的方式被管理

- 让 Web 端可以直接引用 `@autix/contracts`，让前后端共享同一份常量/类型定义

为什么要在这一节就接共享包：因为一旦前端能在编译期直接依赖共享定义，后面无论是 API 调用、表单校验、DTO 还是工具参数，都可以持续复用同一份定义，避免前后端各自复制粘贴、越走越分叉。

### 2.5.1 初始化 clients/chat-web

这里先用 `create-next-app` 把项目骨架生成出来，确保工作区结构与脚手架产物都“落位正确”。

```
bun create next-app clients/chat-web --yes

```

![image 3.png](assets/03-7627370512372465715-07.webp)
配图仅为示例参考，具体内容以文章说明及实际代码为准。

把 `clients/chat-web/package.json` 调整为（关键点：包名统一、脚本使用 Bun、依赖通过 `workspace:*` 直连共享包）：
...
`clients/chat-web/tsconfig.json`：
...
- `@autix/contracts` 的依赖解析是通的（能 import）
...
```jsx
bun install // 由于在 package.json 中新增了本地的 @autix/contracts，需要重新安装以更新 workspace 引用
bun run dev

```

当前分支里 `@autix/chat-web` 的 `dev` 脚本会等待 `4001` 端口可用，因此推荐用根命令 `bun run dev` 同时启动 `@autix/chat` 与 `@autix/chat-web`。如果只启动 Web，需先在另一个终端运行 `bun run dev:chat`，再运行 `bun run dev:chat-web`。

![image 4.png](assets/03-7627370512372465715-08.webp)
配图仅为示例参考，具体内容以文章说明及实际代码为准。

✅ 验收标准

打开 `http://localhost:3002`，页面能显示 `llm`；下一节的 `@autix/chat` 启动后，点击“调用 API”能显示 `/hello` 返回内容。

- 参考资料

- create-next-app: [https://nextjs.org/docs/app/getting-started/installation](https://nextjs.org/docs/app/getting-started/installation)

- transpilePackages: [https://nextjs.org/docs/app/api-reference/config/next-config-js/transpilePackages](https://nextjs.org/docs/app/api-reference/config/next-config-js/transpilePackages)

## 2.6 服务端：初始化 Nest 并提供最小 API

这一节的目标是：

- 初始化 `services/chat`，让 Nest 服务以 workspace 的方式进入 monorepo 任务图

- 提供两个最小接口：`GET /health` 与 `GET /hello`

- 在 `/hello` 里引用 `@autix/contracts`，验证“共享包在服务端同样可用”

这里先不接模型，是刻意的：这一章要先把“服务能跑、路由能通、共享包能引用、前端能调到后端”这条最小链路跑通。后面接什么模型、怎么做工具调用、如何引入运行时，都是在这条链路上加能力，而不是一开始就把问题复杂化。

### 2.6.1 初始化 services/chat

先用 Nest CLI 生成项目骨架，保证目录结构、依赖与默认模块都正确创建。

```
bunx @nestjs/cli new services/chat

```

![image 5.png](assets/03-7627370512372465715-09.webp)配图仅为示例参考，具体内容以文章说明及实际代码为准。

把 `services/chat/package.json` 调整为（关键点：包名统一、脚本与运行方式明确、通过 `workspace:*` 引用 `@autix/contracts`）：
...
### 2.6.3 修改端口到 4001（Nest 默认是 3000）

NestJS 默认监听 `3000`，但本例为了和 Web 的 `3002` 区分，Chat 服务统一使用 `4001`。

在 `services/chat/src/main.ts` 里显式指定端口，并放行本地 Web 的跨域请求：
...
![image 6.png](assets/03-7627370512372465715-10.webp)
配图仅为示例参考，具体内容以文章说明及实际代码为准。

✅ 验收标准

- `curl http://localhost:4001/hello` 返回 `Hello from Chat...`

- `curl http://localhost:4001/health` 返回 `{ "ok": true }`

- 参考资料

- Nest Docs: [https://docs.nestjs.com/](https://docs.nestjs.com/)

- Nest CLI monorepo: [https://docs.nestjs.com/cli/monorepo](https://docs.nestjs.com/cli/monorepo)

## 2.7 前后端联通：Web 调用 Chat 服务并展示结果

这一节的目标是：

- 在 Web 端通过 `fetch` 请求 Chat 服务的 `GET /hello`
...
⚠️ 关键细节：客户端必须通过 NEXT_PUBLIC_ 拿到 API 地址*
...
当 Web 运行在 `http://localhost:3002`、Chat 服务运行在 `http://localhost:4001` 时，浏览器会把它视为跨域请求。下面给两种最小方案，任选其一即可：

#### 方案 A：在 NestJS 里开启 CORS（最简单）

在 `services/chat/src/main.ts` 中：
...
1）在 `clients/chat-web/next.config.ts` 增加 rewrite：
...
这样浏览器看到的是同域请求（`localhost:3002`），不会触发 CORS。

### 2.7.4 运行整条链路

把 Web 和 Chat 服务一起跑起来，用同一条命令从“仓库入口”启动整个最小系统。
...
![image 7.png](assets/03-7627370512372465715-11.webp)
配图仅为示例参考，具体内容以文章说明及实际代码为准。
...
## 2.8 用 Docker Compose 把 Web + Chat 当成一个系统启动

这一节的目标是：

- 写出 `compose.yaml` + `compose.dev.yaml`

- 一条命令启动 chat-web + chat
...
### 2.8.3 Dockerfile.chat 与 Dockerfile.web（镜像构建）

`compose.yaml` 中引用了两个 Dockerfile：一个用于构建 Chat 服务镜像，一个用于构建 Web 镜像。它们都放在 `infra/compose/` 下，但构建上下文是仓库根目录（因为 `context: ../..` 指向仓库根），以便在构建时能访问到 `packages/contracts` 等共享包。

两个 Dockerfile 的共同思路是：

- 以仓库根为构建上下文，完整复制 monorepo（含共享包），让 Bun 在容器里也能按 workspace 解析依赖

- 使用多阶段构建：`deps` 阶段装依赖，`build` 阶段编译产物，最终阶段只保留运行所需文件

- 生产镜像尽量精简，只暴露各自的端口（Chat 4001 / Web 3002）

#### Dockerfile.chat（放在 infra/compose/ 下）
...
#### Dockerfile.web（放在 infra/compose/ 下）

Next.js 在 `next.config.ts` 里已开启 `output: "standalone"` 与 `outputFileTracingRoot`，所以最终镜像只需要拷贝 `.next/standalone` 与 `.next/static` 即可运行，不再需要 `node_modules` 全量。
...
dockerfile 也可以参考生产版本，目前只是demo，不必要现在就运行起来，因为电脑装不了docker，demo 可能有错误。
...
- Multiple Compose files: [https://docs.docker.com/compose/how-tos/multiple-compose-files/](https://docs.docker.com/compose/how-tos/multiple-compose-files/)

- Startup order: [https://docs.docker.com/compose/how-tos/startup-order/](https://docs.docker.com/compose/how-tos/startup-order/)
...
前面 2.4–2.8 我们是手动把工程一步步搭出来的：先把仓库底座跑通，再分别把 Web、Chat 服务、联通、Compose 跑通。
...
- 浏览器打开 `http://localhost:3002`，点击按钮后能看到 Chat 服务返回的 message
...
- 根目录脚本：`dev / dev:chat-web / dev:chat / build / typecheck`

- `bunfig.toml` 开启 isolated installs

- `tsconfig.base.json` 统一 TS 基线，并提供 `@autix/contracts` 的 paths
...
![image 8.png](assets/03-7627370512372465715-12.webp)
配图仅为示例参考，具体内容以文章说明及实际代码为准。
...
- 第二层：把链路跑通（Web ↔ Chat 服务 ↔ 共享包 ↔ Compose）
...
- `clients/chat-web`：能读取共享包内容，并通过按钮触发请求。

- `services/chat`：提供最小可验证接口（`/health`、`/hello`），并能读取共享包内容。

- “联通”不是一句话：你必须在浏览器里看到请求发出、响应回来、页面更新，才算真的跑通。

- `infra/compose`：把多个服务当成一个系统来启动，通过 healthcheck + depends_on 把联调变成可控流程。
...
- 一套能跑起来的最小工程骨架（Web + Chat 服务 + 共享包 + Compose）
