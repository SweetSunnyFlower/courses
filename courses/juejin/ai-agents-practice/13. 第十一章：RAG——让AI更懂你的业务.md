...
原文链接：https://juejin.cn/book/7626976407423303731/section/7627370978134458406

![generated-image-1778897132775.png](assets/13-7627370978134458406-01.webp)

本章demo地址：[feat/rag](https://github.com/Cookieboty/autix-demo/tree/feat/rag)
...
![generated-image-1778897131754.png](assets/13-7627370978134458406-02.webp)
...
![image.png](assets/13-7627370978134458406-03.webp)
...
![image 1.png](assets/13-7627370978134458406-04.webp)
...
| 度量 | 公式 | 含义 |
| --- | --- | --- |
| 欧氏距离 | ![\sqrt{\sum (a_i - b_i)^2}](assets/13-7627370978134458406-05.svg) | 两点之间的直线距离，关心”绝对位置” |
| 余弦相似度 | ![\cos\theta = \dfrac{a \cdot b}{\|a\|\|b\|}](assets/13-7627370978134458406-06.svg) | 两个向量夹角的余弦，关心”方向” |
| 点积 | ![a \cdot b = \sum a_i b_i](assets/13-7627370978134458406-07.svg) | 同方向且都大 → 大；正交 → 0；反方向 → 负 |

为什么文本检索几乎清一色用余弦相似度？两个原因：

(1) Embedding 模型训练时通常做了 L2 归一化

也就是说，模型输出的向量长度都是 1（![\|v\|=1](assets/13-7627370978134458406-08.svg)）。在这种约束下：

![\cos\theta = \dfrac{a \cdot b}{\|a\|\|b\|} = a \cdot b](assets/13-7627370978134458406-09.svg)

而且 ![\|a-b\|^2 = 2 - 2(a \cdot b)](assets/13-7627370978134458406-10.svg)。因此在 Top-K 排序意义上，余弦相似度、点积和欧氏距离通常会给出等价排序：余弦越大，欧氏距离越小。但它们的数值含义并不相同，不能简单说三者“完全相同”。

工程上常用余弦相似度，是因为它直接刻画向量方向，并且输出在 ![[-1, 1]](assets/13-7627370978134458406-11.svg) 区间内，便于跨样本比较。不过在真实 embedding 分布中，分数通常会集中在较窄范围，不一定覆盖完整 ![[-1, 1]](assets/13-7627370978134458406-12.svg)。
...
- 未归一化向量的大小 可能受到文本长度、token 分布、模型内部激活等因素影响，但不应简单解释为“语义强度”
...
![image 2.png](assets/13-7627370978134458406-13.webp)
...
![\mathcal{L} = -\log \dfrac{\exp(\text{sim}(q, k^+) / \tau)}{\exp(\text{sim}(q, k^+) / \tau) + \sum_{i=1}^{N} \exp(\text{sim}(q, k_i^-) / \tau)}](assets/13-7627370978134458406-14.svg)

直观解释：

- ![q](assets/13-7627370978134458406-15.svg) 是 query，![k^+](assets/13-7627370978134458406-16.svg) 是正样本，![k_i^-](assets/13-7627370978134458406-17.svg) 是负样本

- 分子是”和正样本的相似度”，分母是”和正样本 + 所有负样本相似度的总和”

- 训练目标 = 最大化这个比值 = 让正样本相似度远高于所有负样本

- ![\tau](assets/13-7627370978134458406-18.svg) 是温度（temperature），控制”区分难度”，越小越严格
...
![image 3.png](assets/13-7627370978134458406-19.webp)
...
![image 4.png](assets/13-7627370978134458406-20.webp)
...
- Encoder 内部的注意力机制可以直接看到 Q 和 D 的每个 token 互相对照
...
![image 5.png](assets/13-7627370978134458406-21.webp)
...
![image 6.png](assets/13-7627370978134458406-22.webp)
...
![image 7.png](assets/13-7627370978134458406-23.webp)
...
![generated-image-1778897132829.png](assets/13-7627370978134458406-24.webp)
...
![image 8.png](assets/13-7627370978134458406-25.webp)
...
![image 9.png](assets/13-7627370978134458406-26.webp)
...
![image 10.png](assets/13-7627370978134458406-27.webp)
...
![image 11.png](assets/13-7627370978134458406-28.webp)
...
![\text{Recall@K} = \dfrac{\text{Top-K 中相关文档数}}{\text{知识库中所有相关文档数}}](assets/13-7627370978134458406-29.svg)
...
![\text{MRR} = \dfrac{1}{|Q|} \sum_{q \in Q} \dfrac{1}{\text{rank}_q^{\text{first relevant}}}](assets/13-7627370978134458406-30.svg)
...
![\text{DCG@K} = \sum_{i=1}^{K} \dfrac{\text{rel}_i}{\log_2(i+1)}](assets/13-7627370978134458406-31.svg)
![\text{NDCG@K} = \dfrac{\text{DCG@K}}{\text{IDCG@K}}](assets/13-7627370978134458406-32.svg)
...
- RAGAS（[Ragas 文档](https://docs.ragas.io/)）：Python 库，用另一个 LLM 当 judge 自动算 faithfulness / answer_relevancy / context_precision / context_recall
...
![image.png](assets/13-7627370978134458406-33.webp)
实战建议：

- 评测集规模 至少 50 条，否则方差太大不能反映真实情况

- 评测集要覆盖真实业务问题分布（FAQ、长尾、对抗性问法）

- judge LLM 用强模型（GPT-4o / Claude Sonnet），用 RAG 自身的 base 模型会有 bias
...
![image.png](assets/13-7627370978134458406-34.webp)
种子来源：
...
![image 12.png](assets/13-7627370978134458406-35.webp)

## 11.8 提升召回率的策略

11.7 给出了”评什么”的指标。但当指标不达标——召回率只有 60% 而你需要 90% 时，如何改进？这一节是 RAG 工程优化的核心战场。

![image 13.png](assets/13-7627370978134458406-36.webp)
...
export async function rewriteQuery(
  ...
  const result = await model.withStructuredOutput(REWRITE_SCHEMA).invoke([
...
![image.png](assets/13-7627370978134458406-37.webp)
合并策略：
...
![\text{TF-IDF}(t, d, D) = \text{TF}(t, d) \times \log \dfrac{|D|}{|\{d \in D : t \in d\}|}](assets/13-7627370978134458406-38.svg)
...
![\text{BM25}(q, d) = \sum_{t \in q} \text{IDF}(t) \cdot \dfrac{f(t, d) \cdot (k_1 + 1)}{f(t, d) + k_1 \cdot \left(1 - b + b \cdot \dfrac{|d|}{\text{avgdl}}\right)}](assets/13-7627370978134458406-39.svg)

其中：

- ![f(t, d)](assets/13-7627370978134458406-40.svg) = 词 t 在文档 d 中的频次

- ![|d|](assets/13-7627370978134458406-41.svg) = 文档 d 的长度

- ![\text{avgdl}](assets/13-7627370978134458406-42.svg) = 平均文档长度

- ![k_1](assets/13-7627370978134458406-43.svg)（词频饱和参数，典型值 1.2–2.0）

- ![b](assets/13-7627370978134458406-44.svg)（长度归一化强度，典型值 0.75）

BM25 在 TF-IDF 上做了两个关键改进：

1. 词频饱和：通过 ![\dfrac{f \cdot (k_1+1)}{f + k_1}](assets/13-7627370978134458406-45.svg) 让 TF 不再线性增长。当 ![k_1=1.5](assets/13-7627370978134458406-46.svg) 时，f 从 1 到 10 的得分不是 10 倍而是约 2.7 倍——符合直觉。

2. 文档长度归一化：分母里加入 ![b \cdot \dfrac{|d|}{\text{avgdl}}](assets/13-7627370978134458406-47.svg)，让长文档的 TF 被”打折”，避免长文章靠堆字数获胜。
...
![\text{RRF}(d) = \sum_{r \in R} \dfrac{1}{k + \text{rank}_r(d)}](assets/13-7627370978134458406-48.svg)

其中 ![R](assets/13-7627370978134458406-49.svg) 是所有的排序列表（向量 / BM25），![\text{rank}_r(d)](assets/13-7627370978134458406-50.svg) 是 d 在列表 r 中的排名（1-indexed），![k](assets/13-7627370978134458406-51.svg) 是常数（典型值 60）。
...
![image.png](assets/13-7627370978134458406-52.webp)
...
![image.png](assets/13-7627370978134458406-53.webp)
...
- 问题通常短（10 字）：「企业版用户上限是多少」

- 文档通常长（500 字）：「在企业版中，单个工作区最多可创建 200 个用户……配额限制详见……」
  ...
  const hypothetical = await model.invoke([
    {
      role: "system",
      content:
        "请用一段简短的事实陈述回答下面的问题。如果不知道，编一个看起来合理的答案。50-150 字。",
    },
...
![image.png](assets/13-7627370978134458406-56.webp)
...
export async function selfRagAsk(
  ...
  if (relevantChunks.length === 0) {
    return {
      ...
      reflection: { retrieved: true, relevant: 0 },
    };
  }
...
![image.png](assets/13-7627370978134458406-57.webp)
...
export async function cragAsk(
  ...
  const scored = await Promise.all(
    chunks.map(async (c) => ({
      chunk: c,
      score: await evaluator.score(question, c.content),
    })),
  );
...
![image.png](assets/13-7627370978134458406-58.webp)
...
![image.png](assets/13-7627370978134458406-59.webp)
...
async function graphRagAsk(
  question: string,
  graphIndex: {
    searchCommunities: (q: string) => Promise<Community[]>;
    searchChunks: (q: string) => Promise<SearchResult[]>;
  },
  model: BaseChatModel,
): Promise<string> {
...
![image.png](assets/13-7627370978134458406-60.webp)
...
![image 16.png](assets/13-7627370978134458406-61.webp)
...
![image 17.png](assets/13-7627370978134458406-62.webp)
...
![image.png](assets/13-7627370978134458406-63.webp)
...
export function buildFunctionalExpertTools(
  model: BaseChatModel,
  userId: string,
) {
  return [
...
![image.png](assets/13-7627370978134458406-64.webp)
...
![image.png](assets/13-7627370978134458406-65.webp)
...
![image 18.png](assets/13-7627370978134458406-66.webp)
...
![image.png](assets/13-7627370978134458406-67.webp)
...
- 召回质量看板：top1_score、avg_score 趋势图（突然下降 = 知识库可能有问题）
...
![image 19.png](assets/13-7627370978134458406-68.webp)
...
![image.png](assets/13-7627370978134458406-69.webp)
...
修复：用 [pg_jieba](https://github.com/jaiminpan/pg_jieba) 扩展或 Elasticsearch + IK 分析器。
...
![image.png](assets/13-7627370978134458406-70.webp)
...
![image 20.png](assets/13-7627370978134458406-71.webp)
...
- Embedding 模型选型（11.3）：MTEB 看检索栏 + 中英文对齐；BGE / OpenAI / E5 / MiniLM 选型决策树；Bi-Encoder 检索 vs Cross-Encoder 重排的本质区别；换模型必须存 `modelName` 并全量重建。

- 文档切分（11.4）：固定字数 / 递归 / 句子 / 语义 / Parent-Child 五种策略；chunk_size=500、overlap=50 是合理 baseline；中文 separators 要显式覆盖全角标点。
