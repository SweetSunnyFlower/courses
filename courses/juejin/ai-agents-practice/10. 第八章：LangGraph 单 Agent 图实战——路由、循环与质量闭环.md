...
原文链接：https://juejin.cn/book/7626976407423303731/section/7627370589863215142

![image.png](assets/10-7627370589863215142-01.webp)

本章分支[feat/LangGraph](https://github.com/Cookieboty/autix-demo/tree/feat/LangGraph)
...
- 对于线性、可预测、一次性跑完的任务——RAG 问答、结构化抽取、一次性工具调用等——足够简洁优雅。
...
![image 1.png](assets/10-7627370589863215142-02.webp)
...
- 流程是固定的直线或简单分支：LangChain（LCEL）足够。

- 流程包含运行时路径决策、循环、状态共享、人工介入、断点恢复、节点级进度中的任意一项：上 LangGraph。
...
节点就是一个普通的 async 函数：读 `state`，返回要更新的字段（`Partial<State>`）。
...
- 子图：一张 `compile()` 后的图可以直接作为另一张图的节点使用。这是 LangGraph 最重要的组合原语——外层看它仍是一个节点，内部却可以有自己的循环、工具调用和终止逻辑。
...
![image 2.png](assets/10-7627370589863215142-03.webp)
...
![image 3.png](assets/10-7627370589863215142-04.webp)
...
打开 `http://localhost:3000`，登录后创建新对话。
...
![image 4.png](assets/10-7627370589863215142-05.webp)
...
![image 5.png](assets/10-7627370589863215142-06.webp)
...
![image 6.png](assets/10-7627370589863215142-07.webp)
...
![image 7.png](assets/10-7627370589863215142-08.webp)
...
![image.png](assets/10-7627370589863215142-09.webp)
...
![image 8.png](assets/10-7627370589863215142-10.webp)
...
Critic-Refine 与 ReAct 同样依赖回边原语，但两者的回边指向位置和循环目的完全不同：
...
![image 9.png](assets/10-7627370589863215142-11.webp)
...
refineNode 的核心价值是只改有问题的部分，而不是重新生成：
...
![image.png](assets/10-7627370589863215142-12.webp)
...
2. 是否实现了“无 tool_calls”或“critique 为空”的退出分支
...
![image 10.png](assets/10-7627370589863215142-13.webp)

💡 一句话总结

LangGraph 不是为了把简单的事搞复杂，而是让本来就复杂的控制流变得好写、好看、好测、好恢复。直线能搞定的问题就别画图；但一旦涉及分流、循环、质量闭环或人工介入，图结构就比手写 if-else 靠谱得多。
