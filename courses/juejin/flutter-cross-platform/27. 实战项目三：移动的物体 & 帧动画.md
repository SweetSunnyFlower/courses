...
原文链接：https://juejin.cn/book/7178741001677176836/section/7181704043226464316

在上一讲中，游戏世界已经初步构建完成了。现在有静止的背景，以及能向左不停移动的陆地。游戏的主角：小鸟该亮相了，有请主角登场！

![小鸟飞行.gif](assets/27-7181704043226464316-01.gif)
...
![image-20230213150141851.png](assets/27-7181704043226464316-02.webp)

看到了吗？这三张图就是构成小鸟自身动画的全部资源。聪明的你一定发现了其中的奥秘：想让小鸟不停扇动翅膀，那就不停地切换这三张图不就行了吗？这和传统动画片的原理是一模一样的！

![动画原理.gif](assets/27-7181704043226464316-03.gif)
...
和 SpriteComponent 不同，SpriteComponent 只需要在`onload()`方法中初始化一个 sprite。而 SpriteAnimationComponent 则是要初始化一组 sprite。在数据结构层面，表现为 List。废话不多说，直接上代码：
...
阅读这段代码，会发现我还构建了 animation 变量。它通过`SpriteAnimation.spriteList()`赋值，并传入一组 Sprite 和 stepTime，后者表示切换图片的间隔时间。如此，便可得到一只持续扇动翅膀的小鸟吗？还不行，我们还没有定义小鸟出现的位置呢。
...
先来考虑第一个场景。position 表示元素的位置。让小鸟反复上下移动，其实就是在 y 轴方向产生位移。所以，我把这部分的代码实现如下：
...
再来说自由下落，就很容易理解了。我先定义一个 Y 轴方向上的加速度，然后让加速度不停地累加，就能达到越落越快的效果，这也是最贴近真实世界的效果。另外，还要考虑小鸟到达屏幕顶点和落地的处理。相关代码片段如下：
...
到此，我们已经搞定了小鸟所有的运动方式，下面就要让陆地的移动和小鸟的状态联动了。
...
一上来，即游戏前，小鸟自行移动，陆地也跟着动；点击屏幕，小鸟自由下落，此时再点击屏幕，小鸟获得向上加速度，陆地一直动；小鸟落地后，游戏结束，陆地静止不动。
...
又因为游戏的状态和小鸟的状态紧密相连，我们就可以复用小鸟的状态来表示游戏的状态了。
...
陆地的移动逻辑在 LandComponent 中，上一讲我暂且用触摸事件来控制陆地的移动与否。这里就不同了，是游戏的状态，也就是小鸟的状态来决定陆地移动的状态。
