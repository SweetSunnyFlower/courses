  ...
  <div :class="$style.player">
    ...
    <div :class="$style.controls">
      <button :class="$style.iconBtn" @click="skip(-15)" title="后退 15 秒" aria-label="后退 15 秒">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 4 12l7 7"/><path d="M4 12h14"/><text x="13" y="16" font-size="8" stroke="none" fill="currentColor" font-family="sans-serif">15</text></svg>
      </button>
      <button :class="$style.playBtn" @click="toggle" :aria-label="playing ? '暂停' : '播放'">
        <svg v-if="!playing" viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M7 5.5v13a1 1 0 0 0 1.54.84l10-6.5a1 1 0 0 0 0-1.68l-10-6.5A1 1 0 0 0 7 5.5z"/></svg>
        <svg v-else viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><rect x="6" y="5" width="4" height="14" rx="1"/><rect x="14" y="5" width="4" height="14" rx="1"/></svg>
      </button>
      <button :class="$style.iconBtn" @click="skip(15)" title="前进 15 秒" aria-label="前进 15 秒">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 5l7 7-7 7"/><path d="M20 12H6"/><text x="3" y="16" font-size="8" stroke="none" fill="currentColor" font-family="sans-serif">15</text></svg>
      </button>
    </div>
    ...
    <div :class="$style.footer">
      <div :class="$style.rates">
        <button
          v-for="r in rates"
          :key="r"
          :class="[$style.rateBtn, rate === r && $style.active]"
          @click="rate = r"
        >{{ r }}x</button>
      </div>
    </div>
...
.eyes[data-on="1"] span:nth-child(2) { animation-delay: .2s; }
.eyes[data-on="1"] span:nth-child(3) { animation-delay: .4s; }
.eyes[data-on="1"] span:nth-child(4) { animation-delay: .6s; }
.eyes span:nth-child(1) { height: 6px; }
.eyes span:nth-child(2) { height: 12px; }
.eyes span:nth-child(3) { height: 8px; }
.eyes span:nth-child(4) { height: 10px; }
@keyframes eq {
...
.progress:active .thumb { transform: translateY(-50%) scale(1); }
...
.rateBtn:hover { color: var(--vp-c-text-1, #1f2329); }
.rateBtn.active {
...
@media (max-width: 480px) {
  .player { width: calc(100vw - 24px); right: 12px; bottom: 12px; }
}
</style>
