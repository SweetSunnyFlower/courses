...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/site-authority-deployment/12527
...
我们将使用授权策略的 [策略过滤器](https://learnku.com/docs/laravel/9.x/authorization#policy-filters) 机制来实现统一授权的目的。我们只需在策略中定义一个 `before()` 方法。`before` 方法会在策略中其它所有方法之前执行，这样提供了一种全局授权的方案。在 `before` 方法中存在三种类型的返回值：
