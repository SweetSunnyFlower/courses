...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/seo-friendly-url/12515
...
[Guzzle](https://github.com/guzzle/guzzle) 库是一套强大的 PHP HTTP 请求套件，我们将使用 Guzzle 的 HTTP 客户端来请求 [百度翻译](http://api.fanyi.baidu.com/api/trans/product/apidoc) 接口。

Laravel 框架默认依赖于 guzzle，因此我们无需做任何操作即可使用：
...
`app()` 允许我们使用 [Laravel 服务容器](https://learnku.com/docs/laravel/9.x/container) ，此处我们用来生成 SlugTranslateHandler 实例。

测试一下，成功发布一篇文章以后：

![file](assets/12515-03.webp)

数据库查看我们最新发布的一条数据（没看到数据的同学 ctrl + r 刷新）：

![file](assets/12515-04.webp)
...
URI 参数 `topic` 是 [『隐性路由模型绑定』](https://learnku.com/docs/laravel/9.x/routing#%E9%9A%90%E5%BC%8F%E7%BB%91%E5%AE%9A) 的提示，将会自动注入 ID 对应的话题实体。
...
![SEO 友好的 URL](assets/12515-05.webp)

>

注：模板缓存文件不需要修改，会随着每一次 .blade.php 模板文件更改而重新缓存。
...
所有地方修改完成后，打开 [larabbs.test/topics](http://larabbs.test/topics) 查看链接情况：

![file](assets/12515-06.gif)
