...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/new-posts/12508
...
![file](assets/12508-02.webp)

## 提交表单

随便写点内容，然后测试下提交表单，会出现以下报错：

![新建话题](assets/12508-03.webp)
...
- `$request->all()` 获取用户请求的所有数据组成的数组，如 `['title' => '标题', 'body' => '内容', ... ]`；
...
![file](assets/12508-04.webp)

## 模型观察器

`excerpt` 字段存储的是话题的摘录，将作为文章页面的 `description` 元标签使用，有利于 SEO 搜索引擎优化。摘录由文章内容中自动生成，生成的时机是在话题数据存入数据库之前。我们将使用 Eloquent 的 [观察器](https://learnku.com/docs/laravel/9.x/eloquent#observers) 来实现此功能。
