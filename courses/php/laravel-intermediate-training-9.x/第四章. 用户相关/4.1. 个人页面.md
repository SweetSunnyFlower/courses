...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/personal-page/12489
...
我们使用 Laravel 的 [资源控制器](https://learnku.com/docs/laravel/9.x/controllers#resource-controllers) 功能，接下来我们先给控制器注册一个资源路由：
...
此功能称为 [『隐性路由模型绑定』](https://learnku.com/docs/laravel/9.x/routing#%E9%9A%90%E5%BC%8F%E7%BB%91%E5%AE%9A)，是『约定优于配置』设计范式的体现，同时满足以下两种情况，此功能即会自动启用：
...
当请求 [larabbs.test/users/1](http://larabbs.test/users/1) 并且满足以上两个条件时，Laravel 将会自动查找 ID 为 1 的用户并赋值到变量 `$user` 中，如果数据库中找不到对应的模型实例，会自动生成 HTTP 404 响应，例如此刻我们只注册了 Summer 和 Monkey 用户，数据库里只有两条数据，当我们访问 [larabbs.test/users/3](http://larabbs.test/users/3)  ID 为 3 的用户时：

![个人页面](assets/12489-01.webp)
...
接下来我们访问有数据的  1 号用户 [larabbs.test/users/1](http://larabbs.test/users/1) 。由于我们还没有创建用户个人页面，因此这时访问用户页面时会出现如下报错。

```
View [users.show] not found.
```

![个人页面](assets/12489-02.webp)
