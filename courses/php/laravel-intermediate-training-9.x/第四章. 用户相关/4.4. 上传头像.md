...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/upload-avatar/12492

## 上传头像

目前为止，图中的这两张图都是测试图片，接下来我们将一起开发个人资料里的头像上传功能，并将这两张图片换为用户上传的头像。

![file](assets/12492-01.webp)

## 模型文件修改

首先我们需在 `User` 模型里将 `avatar` 字段加入到允许修改的白名单 `$fillable` 中：

![上传头像](assets/12492-02.webp)
...
![file](assets/12492-03.webp)

在 Laravel 中，我们可直接通过 [请求对象（Request）](https://learnku.com/docs/laravel/9.x/requests#retrieving-uploaded-files) 来获取用户上传的文件，如以下两种方法：
...
![file](assets/12492-04.webp)

打印出来是图片文件名称的字符串，与我们的预期不符，图片上传后通过 `$request` 取到的应是图片对象。

经过一番仔细检查后，发现是因为我们忘记为表单添加 `enctype="multipart/form-data"` 声明了。
...
![上传头像](assets/12492-05.webp)

这次对了，可以看到是一个对象打印出来。Laravel 的『用户上传文件对象』底层使用了 UploadedFile 对象进行渲染，为我们提供了便捷的文件读取和管理接口，我们将在后面使用这些方法。
...
![file](assets/12492-06.webp)

提交成功，打开项目文件夹，一步步寻找下去，找到我们刚刚上传的图片：

![file](assets/12492-07.webp)
