...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/delete-reply/12524
...
![file](assets/12524-01.webp)

确定提交后会报错：

![删除回复](assets/12524-02.webp)
...
![file](assets/12524-04.webp)

第二种，寻找其他用户发布的帖子，发布一个评论，可以看到只能删除自己发表的评论：

![file](assets/12524-05.webp)
...
注意: 在模型监听器中，数据库操作需避免再次触发  Eloquent 事件，以免造成联动逻辑冲突。所以这里我们使用了 DB 类进行操作。

测试一下删除话题后，是否回复同时被删除：

1. 找到一篇当前登录用户发布的文章，从浏览器中获取 ID ：

![file](assets/12524-06.webp)

1. 数据库里定位到此篇文章的评论：

![file](assets/12524-07.webp)

1. 删除文章后刷新数据库：

![file](assets/12524-08.webp)
