...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/replies-list/12519
...
![file](assets/12519-01.webp)

接下来优化下样式，修改前请注意 `npm run watch-poll` 已经跑起来：
...
`recent()` 方法在数据模型基类 `app/Models/Model.php` 中定义，并且使用了 [本地作用域](https://learnku.com/docs/laravel/9.x/eloquent#local-scopes) 的方式进行定义，我们的 Reply 模型，就如代码生成器所生成的数据模型一样，统一继承了此类方法：
