...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/message-notification/12521
...
打开 `User.php` 文件，将 Notifiable 的引入修改如下：
...
![file](assets/12521-01.webp)

访问登录页面，黏贴进去邮箱，密码是通用的 `password` ：

![file](assets/12521-02.webp)

登录成功后，访问 Summer 的个人空间 [larabbs.test/users/1](http://larabbs.test/users/1) ，找一条 Summer 发布过的话题进行回复：

![file](assets/12521-03.webp)

刷新数据库即可看到 `notifications` 表里多了一条数据，并且 `notifiable_id` 为 1 ，也就是 1 号用户 Summer 有一条新通知：

![file](assets/12521-04.webp)
