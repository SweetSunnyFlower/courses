...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/user-last-logon-time/12538
...
我们可以使用 Redis 来记录用户的访问时间，Redis 运行在机器的内存上，读写效率都极高。不过为了保证数据的完整性，我们需要定期将 Redis 数据同步到数据库中，否则一旦 Redis 出问题或者执行了 Redis 清理操作，用户的『最后活跃时间』将会丢失。
...
我们将利用 [Laravel 中间件](https://learnku.com/docs/laravel/9.x/middleware) 功能来实现用户访问时间的记录。
...
我们将业务逻辑封装于 User 类中，事实上，我们要将记录和读取的相关逻辑放置于单独的 Trait 中：
...
我们将使用 Eloquent 的 [访问器](https://learnku.com/docs/laravel/9.x/eloquent-mutators#defining-an-accessor) 来实现此功能。当我们从实例中获取某些属性值的时候，访问器允许我们对 Eloquent 属性值进行动态修改。
...
![file](assets/12538-04.webp)

![file](assets/12538-05.webp)
