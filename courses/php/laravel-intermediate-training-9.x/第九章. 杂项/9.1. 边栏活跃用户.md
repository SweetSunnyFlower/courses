...
原文链接：https://learnku.com/courses/laravel-intermediate-training/9.x/active-users/12535

## 活跃用户

![file](assets/12535-01.webp)
...
首先顶部导入 User 类，并作为 `index()` 的第三个参数传参，这里只是一个便捷的 `$user = new User` 写法而已。

访问话题列表页面，即可看到我们的『活跃用户』集合：

![file](assets/12535-03.webp)

测试完成后请将 `dd($active_users);` 删除。

## 4. 页面展示

我们已经可以读取到数据，接下来我们在右边栏中把数据显示出来。
...
5. 键盘输入 `:wq` 并敲击回车键保存退出。

整个操作过程的动图如下：

![file](assets/12535-05.gif)
